...
if (!isset($_COOKIE['username'])) {
    ...
    $query = "SELECT * FROM blitz WHERE `username` = '$current_username'";
    $result = mysqli_query($conn, $query);
    
    if (mysqli_num_rows($result) > 0) {
        // The user's record exists in the database, so you can proceed with the deletion
        $delete_query = "DELETE FROM blitz WHERE `username` = '$current_username'";
        
        if(mysqli_query($conn, $delete_query)){
            echo '<script>
                alert("You have successfully removed your name from the Blitz Tournament");
                window.location.href = "category_page.php";
            </script>';
        } else {
